...
public class Facture {
	...
	public String resumPanier(Map<Produit, Integer> ListePanier) {
		...
		return "Le total de la commande est de " + totalProduits(ListePanier) + "  €";
	}
	
	public StringBuilder TPanier(Map<Produit, Integer>ListePanier) {
		for(Entry<Produit, Integer> element : this.ListePanier.entrySet()) {
		s.append("- ").append(element.getKey().getNom()).append(" - ").append(element.getKey().getPrix()).append("  €").append("<br>");
		}
		return s;
	}
	...
	public void generateConsole() {
		...
		System.out.println(" La Pepinière");
		System.out.println("");
	...
	public StringBuilder afficherFacture(Map<Produit, Integer> produits) {
		...
		str.append("Client : ").append(client.getNom() + " " + client.getPrenom()).append("<br><br>");
		str.append("------------------- PRODUITS -------------------").append("<br><br>");
		str.append(TPanier(produits));
		str.append("TOTAL : ").append(this.totalProduits(produits)).append(" €").append("<br>");
